...
def test_volumetric_edges_api_returns_expected_link_count(tmp_path):
    results = extract(
        SUBJECTS,
        VBM_DIR,
...
def test_volumetric_roi_stats_api_returns_expected_node_count(tmp_path):
    ...
    assert sorted(results) == ["CAM_0002_01"]
    assert results["CAM_0002_01"].size == NUM_ROI_ATLAS["cat_aal"]


def test_volumetric_cli_supports_builtin_atlas_names(tmp_path):
    run_dir = cli_run(
        [
            "edges",
            "-i",
